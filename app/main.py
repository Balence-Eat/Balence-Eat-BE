...
async def signup(user: user_schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    new_goal = models.Goal(
        user_id=new_user.user_id, weight=user.goal.weight, date=user.goal.date
    )
    db.add(new_goal)
    db.commit()
    db.refresh(new_goal)
...
@app.patch("/profile/edit-profile")
def edit_profile(
...
def add_meal(
    ...
    for item in meal.items:
        ...
        if not food:
            raise HTTPException(
                status_code=404, detail="해당 음식이 존재하지 않습니다."
            )
...
def get_meals(
    ...
    if date:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="날짜 형식은 YYYY-MM-DD여야 합니다."
            )
    if meal_type:
...
def get_ai_diet(
    ...
    total_eaten = sum(
        (
            ...
            .filter_by(food_id=meal.food_id)
            .first()
            .calories_per_unit
            or 0
        )
        * meal.quantity
        for meal in meals
        if db.query(models.Food).filter_by(food_id=meal.food_id).first()
    )
    inventory = db.query(models.UserFoodInventory).filter_by(user_id=user.user_id).all()
    food_items = [
        {
            "name": db.query(models.Food).filter_by(food_id=inv.food_id).first().name,
            "quantity": inv.quantity,
        }
        for inv in inventory
        if db.query(models.Food).filter_by(food_id=inv.food_id).first()
    ]
